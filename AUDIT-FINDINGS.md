...
| 19 | **The department block has no “not sure yet.”** The company stage has all three parts of that mechanism — `CompanyAnswer.value` is `str \| None`, it carries an `unsure` flag, and the route runs every answer through `resolve_answer` so a blank stores the question's stated assumption flagged as one. The block has none of them: `BlockAnswer.value` is a bare `str`, there is no `unsure` field, the POST calls `store_answer` directly, and **all 29 questions in the bank have `assumption_when_unsure = None`** | `routes/spine.py:282`, `domain/question_bank.py` | **Open.** These are the questions most likely to be unknown — payment terms, approval thresholds, runway alerts — and the only way past them is to type something. A product that sells on *never invent a number* should not make inventing one the sole path through its own capture form. Needs an assumption per question (Q33's shape) before the plumbing is worth adding |
| 20 | **A stored block answer is never returned, so it cannot be seen or corrected.** `BlockQuestionOut` carries `answered: bool` and `proposed: bool` but not the value. The UI renders an ANSWERED badge beside an empty textarea; re-saving overwrites what the founder can no longer read. `QuestionOut` in `routes/setup.py` already has `value` with a docstring about exactly this | `routes/spine.py:206` | **Open.** Confirmed in the browser against Neon, not inferred from the type |
| 21 | **The block and the dashboard are served for departments the workspace never chose.** The block's 404 asks only whether the *bank* has questions for that department, never whether this company runs it. A workspace on finance/sales/operations gets a full HR block with `may_answer: true`, and `/dashboards/hr` returns a director that `/dashboards` does not list. Answers written there are unreachable from every surface | `routes/spine.py:243`, `routes/dashboards.py:256` | **Open.** Not a tenancy leak — same workspace, owner role — but two endpoints that contradict each other about what exists, and a way to write rows nothing will ever read |
...
| 23 | **`GET /dashboards` exceeds the BFF's 15-second timeout against Neon.** The primary dashboard 503s in the browser while `/dashboards/finance` returns. Measured from this machine: ~0.35s per round trip, and the list spends 25–30 of them; `/auth/session` spends about twelve on a single session lookup, and a pooled checkout costs 1.9s | `apps/web/app/api/dashboards/route.ts`, `routes/dashboards.py:197` | **Open.** CI's local Postgres answers in microseconds, so nothing catches it there. This is the third instance of the recurring class — a drifted database, a proxy that filtered GUCs, an unpinned resolver, and now a latency budget that only production pays |
