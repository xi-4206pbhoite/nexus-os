...
| 24 | **The deployed stack cannot send email, so it cannot be deployed.** `NEXUS_ENV=production` forbids `NEXUS_MAILER_BACKEND=file` — correctly, since a production process writing `.eml` files to disk sends no mail — and there is no other backend configured. `migrate` refuses to start, and so would `api`. Found by building and running the images in CI, which is the first time any of this ran | `config.py`, `docker-compose.yml` | **Open.** This puts **D4** (the email provider) on the critical path for a deployable stack. D4 was closed in P5's walkthrough on the grounds that `FileMailer` made the whole verification chain work with no provider — true for development, and it is exactly what `production` refuses. Needs a mail-catcher service for CI and a real provider for production; P9's browser journey cannot pass until one exists, because it reads the verification link out of the mail that was actually sent |
