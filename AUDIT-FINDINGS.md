...
| 18 | **"First verified wins" has never worked, and two people can each verify the same domain** - `create_workspace_for_claim` detects a rival with `SELECT id FROM workspace WHERE lower(domain) = :d AND domain_verified_at IS NOT NULL` on an **unscoped** session. `workspace` carries FORCE RLS with two policies: `workspace_id = nexus.workspace_id` (migration 0002) and "a workspace you hold a membership in" (0008). A second claimant matches **neither**, so the query returns nothing, the dispute branch never runs, and creation proceeds. Measured against the live database, not reasoned: a second claimant sees NOTHING with no GUC and NOTHING with their own `user_id` set, while the incumbent owner sees FOUND. **No test has ever covered the dispute path** | `auth/domains.py:243` | Open. Found in Phase 5 when the same query, reused for the duplicate-domain branch, failed its new test. The partial unique index on `lower(domain) WHERE domain_verified_at IS NOT NULL` from migration 0002 is what has been holding the line - so the second verification fails on a constraint violation rather than producing two workspaces, and the user sees a 500 instead of "that company is already here". Needs the same shape of answer as D24 |
